...
if ($billingEntry.MatterId -gt 0 -and $billingEntry.UnitPrice -eq 0) {
  ...
  $billingProfile = Get-MatterBillingProfile $billingEntry.MatterId

  # Get the billing type from the matter profile
  $billingType = $billingProfile | Where-Object { $_.BillingTypeId -eq $billingEntry.BillingTypeId }
  ...
  if ($null -eq $billingType) {
    ...
    if ($null -eq $agilityBlueObject) {
      Set-BillingEntry $billingEntry -BypassCustomFieldValidation $true | Out-Null
